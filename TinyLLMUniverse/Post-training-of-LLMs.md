...
- **参数高效微调**：常用的轻量化微调技术有LoRA、Adapter 和 Prompt Tuning 。例如 LoRA（低秩适配）通过在每层引入小的低秩矩阵 A 和 B 来调整模型参数。这减少了可训练参数的数量，节省显存，缺点是学习和遗忘都更有限，因为更新的参数更少。
...
- [【深度学习】混合精度训练与显存分析](https://zhuanlan.zhihu.com/p/608634079)
  ...
  <img width="400" height="300" alt="image" src="https://github.com/user-attachments/assets/4d724c69-0a1e-4ee9-bdd4-78d8ec771279" />
...
### Tips 为什么Online RL不容易“破坏”已有能力：

#### 一、从模型分布角度
- **监督微调（SFT）**

它是“强行模仿”——让模型输出尽量贴近人工示例（Example Response）。但这些示例往往不完全覆盖模型原先分布，也可能包含一些风格或表达方式上的偏差。于是优化会强制模型往示例方向拉动参数，即使那方向超出了模型原本的语义空间（native manifold）。

👉 结果：模型的原有“能力子空间”可能被扰动，出现灾难性遗忘（catastrophic forgetting）。

- **在线强化学习（Online RL）** 

它不是模仿，而是“调节行为”——在模型本身的输出空间中试探性地产生多个候选响应，通过奖励函数（Reward）选择更好的方向。因此，它只在模型原有的行为流形中调整权重，属于“在原有能力范围内微调”。

👉 结果：不会偏离原有能力分布，因此更不容易破坏旧能力。
...
$$
\arg\max_\theta \sum_i \log P_\theta(y_i | x_i)
$$

——即最大化示例响应的概率。这种方式会对所有示例强约束，即便某些示例与模型原先偏好相冲突，也会强行“拉过去”（ **强制模仿（Imitation）**）。

- **Online RL的目标函数**是：

$$
\arg\max_\theta \mathbb{E}_{x,y\sim P_\theta}[r(x,y)]
$$

——只鼓励奖励高的响应，而不是强制模仿。所以它对模型行为的干预是方向性的（reward-driven）而非强制性的，因此优化更柔和、更稳。（**奖励驱动的探索（Reward-guided Adjustment）**。）

#### 三、从梯度更新机制角度

SFT：每个样本的梯度更新方向直接影响模型参数，容易导致梯度冲突（gradient interference），使模型“忘掉”原本的模式。

Online RL：通过奖励函数的归一化（如PPO的advantage clipping）控制更新幅度，只微调行为概率，而非大规模重塑参数分布。

👉 梯度更新被“限制在安全范围内”，避免了参数的剧烈漂移。
