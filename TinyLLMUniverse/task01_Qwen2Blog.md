...
即给定前面出现的所有词，模型可以预测下一个词 $𝑥_𝑖$ 的概率。
...
- 较低的困惑度：表示模型对下一个词的预测更准确，也就是模型的预测更具确定性。
- 较高的困惑度：表示模型对下一个词的预测更不确定，说明模型的性能较差。

存在问题：一个序列的联合概率取决于其长度，并且随着长度的增长，其值趋近于零，这使得困惑度变得难以追踪。直观上，希望对每个词标记（token）的概率 $p(x_{i}∣x_{1:i−1})$ 进行平均。这里的 p(xi∣x1:i−1) 表示给定之前的词序列 $x_{1:i−1}$ 后，下一个词 $x_{i}$ 出现的概率。这样做的目的是评估模型在处理各种词标记时的平均性能。
