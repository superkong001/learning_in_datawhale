...
4. Unigram model (SentencePiece工具) 
  给定一个序列 $x_{1:L}$ ，一个分词器 $T$ 是 $p\left(x_{1: L}\right)=\prod_{(i, j) \in T} p\left(x_{i: j}\right)$ 的一个集合。如这个实例：
    ...
    - 训练数据（字符串）： $𝖺𝖻𝖺𝖻𝖼$
    - 分词结果  $T={(1,2),(3,4),(5,5)}$ (其中 $V=\{𝖺𝖻,𝖼\}$ ) 表示将字符串拆分成三个子序列： $(𝖺,𝖻)，(𝖺,𝖻)，(𝖼)$ 。词汇表 $V=\{𝖺𝖻,𝖼\}$ 表示了训练数据中出现的所有词汇。
    - 似然值： $p(x_{1:L})=2/3⋅2/3⋅1/3=4/27$ 是根据 unigram 模型计算得出的概率，表示训练数据的似然度，即将训练数据分词为所给的分词结果 $T$的概率。其中：$p(𝖺𝖻)=2/3$ ， $p(𝖼)=1/3$ 。通过将各个词汇的概率相乘得到整个训练数据的似然值为 $4/27$ 。似然值用于评估分词结果的质量。较高的似然值表示训练数据与分词结果之间的匹配程度较高，这意味着该分词结果较为准确或合理。
    - 算法流程：从一个“相当大”的种子词汇表 $V$ 开始。重复以下步骤：
      a. 给定 $V$ ，使用EM算法优化 $p(x)$ 和 $T$ 。
      b. 计算每个词汇 $x∈V$ 的 $loss(x)$ ，衡量如果将 $x$ 从 $V$ 中移除，似然值会减少多少。
      c. 按照 $loss$ 进行排序，并保留 $V$ 中排名靠前的80%的词汇。
