...
- $ \log \frac{1}{p \left( x_{i} \mid x_{1:i - 1} \right)} $ 对每个词的条件概率 $p \left( x_{i} \mid x_{1:i - 1} \right)$ 取对数，并取倒数。它度量了在模型预测该词时的“不确定性”或“困惑度”。如果模型的预测准确，条件概率会较高，取对数后的值会较低。
...
- $\exp \left( . \right)$ 取指数，得到困惑度值。因为对数的单位是信息量（比如比特），取指数后的结果则是实际的困惑度度量，反映了模型对整个序列的预测不确定性。
与整个序列的联合概率 $Perplexity(X) = P(X)^{(-1/N)} = P(x_1,x_2,...,x_N)^{(-1/N)}$ 是等价的。
