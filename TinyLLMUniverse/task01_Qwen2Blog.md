...
编码器-解码器结构：
- 编码器将输入序列变换为隐藏层特征，任务是“理解”输入的整个句子。它会读取所有输入词元，最终为每个词元生成一个富含上下文信息的向量表示。在编码器中Q,K,V相同,均为自身前一层的输出；
...
- 解码器将隐藏层特征变换为输出序列，任务是“生成”目标句子。它会参考自己已经生成的前文，并“咨询”编码器的理解结果，来生成下一个词。在解码器中Q来自前一层输出,K,V是编码器输出；
...
位置编码的核心思想是，为输入序列中的每一个词元嵌入向量，都额外加上一个能代表其绝对位置和相对位置信息的“位置向量”。这个位置向量不是通过学习得到的，而是通过一个固定的数学公式直接计算得出。这样一来，即使两个词元（例如，两个都叫 $agent$ 的词元）自身的嵌入是相同的，但由于它们在句子中的位置不同，它们最终输入到 Transformer 模型中的向量就会因为加上了不同的位置编码而变得独一无二。原论文中提出的位置编码使用正弦和余弦函数来生成。
...
传统的概率分布使由 Softmax 公式计算得到的： $p_ i = \frac{e^{z_ i}}{\sum_ {j=1}^k e^{z_ j}}$ ，采样参数的本质就是在此基础上，根据不同策略“重新调整”或“截断”分布，从而改变大模型输出的下一个token。

$Temperature$ ：温度是控制模型输出 “随机性” 与 “确定性” 的关键参数。其原理是引入温度系数 $T\gt0$ ,将 Softmax 改写为 $p_ i^{(T)} = \frac{e^{z_ i / T}}{\sum_ {j=1}^k e^{z_ j / T}}$ 。
...
- Encoder output：从编码器得到的输出。
- Feed Forward FFN：前馈神经网络（Feed Forward Neural Network）。
- Multi-Head Attention：多头自注意力机制，用于捕捉输入序列中各个部分之间的关系。
- Gating：门控机制，用于决定哪些专家模型参与计算，哪些专家将被“关闭”。
- Add & Norm：残差连接和归一化操作
...
也就是每个专家都会给出一个评分，然后根据这些评分（权重）来决定每个专家对最终输出的贡献程度。例如，如果只选取前 $Top 2$ 个专家的权重，得到 $\tilde g(x)$ 为：

$$
\tilde g(x)=[0, 0.84, 0.16]
$$
...
- 设 $c_e = \sum_{i=1}^B \mathbf{1}[\tilde g_e(x_i) > 0]$ 是专家 $e$ 被选中的次数。
    -  $\mathbf{1}[\tilde g_e(x_i) > 0]$ 是一个指示函数，表示如果输入 $𝑥_𝑖$ 被专家 $𝑒$ 选择（即 $𝑔_𝑒(𝑥_𝑖)>0$ ），则该函数为 1；否则为 0。
    -  $B$ 是每个批次中的样本数。
    -  $c_e$ 代表在一个批次中，专家 $𝑒$ 被选中的次数。
- 注意，处理完一个batch后， $\sum_e c_e = B$ ，表示所有专家被选择的总次数等于批次中的总样本数 $𝐵$ 。
- 如果所有专家的选择次数相等（所有专家都是平衡的），即每个专家大约被选择 $\frac{B}{E}$ 次，其中 $𝐸$ 是专家的总数。那么， $𝑐_𝑒 = \frac{B}{E}$ 就表示每个专家被均匀选择的情况。
- 溢出：如果 $c_e > 2 \frac{B}{E}$ ，则设 $f(x) = x$ （带残差的旁路），其中2是容量系数。也就是如果 $𝑐_𝑒 \geq \frac{B}{E}$ ，则某个专家被选择的次数超过了平衡的次数，那么会导致计算负担不均，某些专家的负载过重。
- 辅助损失：为了避免这种负载不均的情况，模型需要进行负载均衡，通过某些机制确保每个专家的负载尽可能平衡。为了实现这一点，会在目标函数中加入负载均衡损失项，所以会期望 $c = [c_1, \dots, c_E]$ 接近均匀分布。
- 可以惩罚 $\|c\|_ 2^2 = \sum_{e=1}^E c_e^2$ ，但这是不可微分的。
...
其中 $m_e$ 是专家 $e$ 的负载平衡参数，通过调节 $m_e$ ，可以控制专家的选择频率。通过最小化负载均衡损失项，模型能够将各个专家的负载 $𝑐_𝑒$ 调整到尽可能接近 $\frac{B}{E}$ ，从而避免某些专家过度计算而其他专家被忽视。
