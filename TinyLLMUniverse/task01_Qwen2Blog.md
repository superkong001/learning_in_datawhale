...
熵实际上是一个衡量将样本 $x∼p$ 编码（即压缩）成比特串所需要的预期比特数的度量。如："the mouse ate the cheese" 可能会被编码成 "0001110101"。
...
- 公平的掷硬币（正面和反面概率各为 0.5），那么熵是：
  ...
  $$
    H(𝑋) = − (0.5 \log 2 0.5+0.5 \log 2 0.5) = 1 bit
  $$ 
- 不公平的掷硬币（比如正面 0.9，反面 0.1），那么熵是：
  
  $$
      H(X)= − (0.9 \log 2 0.9+0.1 \log 2 0.1) ≈ 0.468 bits
  $$ 
...
这意味着可以通过构建一个只有来自真实数据分布 $p$ 的样本的（语言）模型 $q$ 来估计 $H(p,q)$ ，而 $H(p)$ 通常无法访问，所以可以通过构建更好的模型q来得到熵H(p)的更好的估计，由H(p,q)衡量。
