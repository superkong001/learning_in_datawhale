...
- 全流程密态保障：基于硬件隔离环境或密码协议，构建机密虚拟机、TEE OS、机密容器、密态胶囊等，结合密态互联实现全链路密态。
  ...
  - 密态胶囊：数据与使用策略（数字合约）绑定胶囊；用户通过加密确保只有指定可信应用能接收到；用户通过检查代码，确认可信应用会严格检查策略。
    ...
    - 组成：密态胶囊由元数据、数据密文、安全信息、使用控制策略组成，并携带数字签名。
      • 元数据：包括数据的唯一标识、持有者、类型、表结构、数据摘要、数据血缘等；
      • 使用控制策略：包括策略配置、策略规则列表等；规则包括描述、授权人、被授权人、效果、生效周期、操作、约束等；
      • 安全信息：KEK 公钥加密后的数据密钥DK密文，以及胶囊持有者；（数据密钥DK用来加密原始数据的对称密钥，保障胶囊数据机密性；KEK公钥用来加密数据密钥DK的公钥，保障胶囊数据密钥的机密性）
      • 数据密文：加密的数据。
    ...
    <img width="350" height="236" alt="image" src="https://github.com/user-attachments/assets/b560c521-d577-4d31-9d87-5e9d1f48eb29" />
 
    <img width="424" height="200" alt="image" src="https://github.com/user-attachments/assets/b27af432-f12f-4b56-85d3-f3705346efab" />
